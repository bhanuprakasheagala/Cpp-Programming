...
int main()
{
    ...
    // Convert std::vector to a compatible type to pass to showMemory function
    unsigned char* vecData = reinterpret_cast<unsigned char*>(v.data()); // Cast vector data pointer to unsigned char*
    std::size_t vecDataSize = v.size() * sizeof(v[0]); // Calculate the size of the vector data
    showMemory(vecData, vecDataSize, "Vector Memory"); // Display the contents of the vector memory
